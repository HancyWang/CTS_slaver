...
{
	//这些flag可以在Is_Timing_Xms这个函数中自己恢复
	...
	PWM_waitBeforeStart_cnt=0; //wait_before_start的秒数
...
//				uint16_t ret=ADS115_readByte(0x90);
				uint16_t ret=adc_pressure_value;
...
//						dutyCycle=(PID_P*pressure_diff>=100?100:PID_P*pressure_diff)+PID_I*integration_sum;
		...
		{
			if(*p_PWM_numOfCycle==buffer[1+ELEMENTS_CNT*(*p_PWM_serial_cnt)+NUM_OF_CYCLES])  //运行完最后一个wait_between
			{
				//注意：这部分的逻辑是定制的，没有通用性
				//后续根据Ilan的要求，可能还会变化
				#if 0
...
//				*p_PWM_numOfCycle=0;
				...
				//新增，组后一个wait_between,也要有，不然没办法开阀放气
				if(num==3)
				{
					Operate_magnetic_valve(MAGNETIC_OPEN); //开阀，wait_after中关掉阀，标识：@1
					*p_pwm_state=PWM_WAIT_AFTER;
					*p_PWM_numOfCycle=0;
					...
					pwm1_state=PWM_START;   
					pwm2_state=PWM_START;
					PWM1_serial_cnt++;  //serial_cnt加1,这样下个波形输出的时候可以在buffer[]数组中偏移到需要的参数位置，不然取不到对于的参数
					PWM2_serial_cnt++;
					PWM1_numOfCycle=0;  //将serial中的num_of_cycle清零
					PWM2_numOfCycle=0;
				}
				else
				{
					//PWM1和PWM2可以不用管，上位机给PWM1和PMW2的period运行200s时间，PWM3运行完一个serial之后都可以保证PWM1/PWM2不会跑到这里来
					*p_pwm_state=PWM_WAIT_AFTER;   //写一下吧，虽然没什么用
					*p_PWM_numOfCycle=0;
				}
			}
			else
			{
				//PWM3在wait_between的时候要开阀放气
				if(num==3)
				{
					if(Is_timing_Xmillisec(buffer[1+ELEMENTS_CNT*(*p_PWM_serial_cnt)+WAIT_BETWEEN]*1000,num))
					{
//					//定制代码,在某个特定的wait_between之后开启PWM1和PWM2
//						//控制马达开始震动
						#if 0
		...
		{	
			if(num==3)
			{
				if(Is_timing_Xmillisec(buffer[1+ELEMENTS_CNT*(*p_PWM_serial_cnt)+WAIT_AFTER]*1000,num))
				{
					...
					Operate_magnetic_valve(MAGNETIC_CLOSE);  //呼应标识：@1,关闭电磁阀
...
//				else
				#endif
				{
					//这里只有PWM3可以运行到，PWM1和PMW2设置的period为200s，到达不了这里
					if(Is_timing_Xmillisec(buffer[1+ELEMENTS_CNT*(*p_PWM_serial_cnt)+WAIT_AFTER]*1000,num))
					{
						state=OUTPUT_PWM;
						*p_PWM_numOfCycle=0;
...
//		Calibrate_pressure_sensor(&zero_point_of_pressure_sensor);  //获取sensor零点值
	...
	if(self_tet_state==SELF_TEST_INFLATE)
	{
		...
		uint16_t res=adc_pressure_value;
//		if(res>=((PRESSURE_RATE)*(183))+zero_point_of_pressure_sensor)
		...
		#endif
		{
			...
			//记录数据2
			selfTest_inflate_record_2=adc_pressure_value;
			//数据2-数据1
...
//				inflate_cnt=0;
			...
			else
			{
				...
				{
					//记录数据1
					selfTest_inflate_record_1=adc_pressure_value;
				}
				#if 1
//				if(inflate_cnt==399)
//				{
...
//			GPIO_SetBits(GPIOB,GPIO_Pin_11);
		...
		else
		{
			...
			{
				//记录数据1
				selfTest_hold_record_1=adc_pressure_value;
			}
			if(hold_cnt==1490)
			{
				//记录数据2
				selfTest_hold_record_2=adc_pressure_value;
				//数据2-数据1
...
//			GPIO_ResetBits(GPIOB,GPIO_Pin_11);
		...
		else
		{
			...
			{
				//记录数据1
				selfTest_deflate_record_1=adc_pressure_value;
			}
			if(deflate_cnt==200)
			{
				//记录数据2
				selfTest_deflate_record_2=adc_pressure_value;
				//数据2-数据1
...
//							memcpy(pwm_buffer,buffer+2,144+6);  //MODE1的PWM1,PWM2,PWM3 
				...
				{
					pressure_result=adc_pressure_value;
